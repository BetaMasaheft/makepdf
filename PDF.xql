...
(
   ...
   let $value :=
     (  if ($node/@from and $node/@to)
                        ...
                        if ($node/@from) then
                            ...
                            then
                                let $targets := 
                                    ...
                                    ((if($fFornot = 'x') then () else ($fFornot||'. ')) 
                                    || substring-after(string($node/@target), '#'))
                                let $cutlistoftargets := if (count($targets) ge 3) then (subsequence($targets, 1, 3), 'etc.') else $targets
                                return
               ...
               $value
                    ,
                    if(($node/following-sibling::element())[1]/name() = 'locus') then ', ' else ()
                   ...
                   (  let $parents := for $msItem in $node/ancestor::tei:msItem
                                                    return
                                                         if($msItem/parent::tei:msItem) then 
                                                           ...
                                                           else local:n2roman(count($msItem/preceding-sibling::tei:msItem) +1)
...
(:                    first order msItem:)
                    ...
                    replace(concat(string-join(fo:tei2fo($node/tei:locus)), ': '), '(\S)(Fol.)', '$1, ') else ()}{<fo:inline xml:lang="en" hyphenate="true">{fo:ContentsTitle($node/tei:title)}</fo:inline>}</fo:inline>
                    {if($node/tei:note) then <fo:block  start-indent="10mm"> {for $i in $node/tei:note return fo:tei2fo($i/node())}</fo:block> else ()}
                   {if($node/tei:incipit) then <fo:block start-indent="10mm"> {'Incipit: ', for $i in $node/tei:incipit return fo:tei2fo($i)}</fo:block> else ()}
                     {if($node/tei:explicit) then <fo:block start-indent="10mm"> {'Explicit: ', for $i in $node/tei:explicit return fo:tei2fo($i)}</fo:block> else ()}
            </fo:block>,
...
<fo:table-body>
  {fo:tei2fo($node/tei:row[not(@role)])}
    ...
    case element(tei:title)
        return
        if($node/ancestor::tei:titleStmt) then fo:tei2fo($node/node()) else  fo:entitiesWithRef($node)
...
<fo:inline page-break-after="avoid">
 ...
 fo:tei2fo($node/node())
                     else if($node[not(@type)]/@cRef) then 
                       if($node/parent::tei:cit) then
                        let $wordcount := count(tokenize(string-join($node/parent::tei:cit/tei:quote[1]//text(), ' '), '\s+'))
                          return
                             ...
                             <fo:inline id="{$refid}">({$node/text()})</fo:inline>
                              else
                            <fo:block page-break-after="avoid" id="{$refid}" text-align="right" hyphenate="true">{$node/text()}</fo:block> 
                       else <fo:inline id="{$refid}">{$node/text()}</fo:inline>
                    ...
                    case 'internal' return (
                     ...
                     case 'manuscript' return 
                              ...
                              let $chapter := collection('/db/apps/BetMasData/manuscripts')//id($chid)
                                return
                                (<fo:basic-link internal-destination="{$chid}">
                                 <fo:inline>{'Ms. ' || $chapter//tei:msIdentifier/tei:idno[not(@xml:lang)]/text()}</fo:inline>, 
                                    p.</fo:basic-link>,
                                <fo:page-number-citation ref-id="{$chid}"/>)
                    case 'figure' return (let $corresp := substring-after($node/@target, '#') 
                                                                                let $figure:= root($node)//tei:*[@xml:id=$corresp]
                                                              return  ('Fig. ' ||(count($figure/preceding::tei:graphic) +1) ))
                    case 'work' return <fo:basic-link external-destination="https://betamasaheft.eu/{$node/@corresp}">{fo:titleSelector($node/@corresp)}</fo:basic-link>
                    default return string($node/@target)} </fo:inline>
                   else if ($node/name() = 'persName' or $node/name() = 'placeName' ) then 
          (if($node/parent::tei:titleStmt) then fo:tei2fo($node/node()) else  fo:entitiesWithRef($node))
             else if ($node[starts-with(@target, '#h')]) then 
            'hand ' || substring-after($node/@target, '#h')
            else if ($node[starts-with(@target, '#')]) then 
                    let $root := $node/ancestor::tei:TEI
                    let $target := substring-after($node/@target, '#')
                    return replace($target, '\s\([a-zA-Z0-9\-_\.\s]+\)', '')
                   else
        ...
        return
            <fo:inline>
                {if ($node/@xml:lang = 'gez' and not(matches($node, '\p{IsEthiopic}'))) then ()
                    else if ($node/@xml:lang = 'ar' and not(matches($node, '\p{IsArabic}'))) then ()
...
declare function fo:table-of-contents() {
    <fo:page-sequence
      ...
      {    for $settingsValue in $local:settings/s:orderOfParts/element()
       ...
       case 'aknowledgments' return <fo:block
                            text-align-last="justify"
                            space-after="1pt"
                            font-size="10.5pt"
                            font-family="Ludolfus" 
                            ...
                            <fo:leader
                                leader-pattern="dots"/>
                            <fo:basic-link internal-destination="Acknowledgement">
                            <fo:page-number-citation
                                ref-id="Acknowledgement"/>
                            </fo:basic-link>
                            
                        </fo:block>       
       case 'listofimages' return <fo:block
                            text-align-last="justify"
                            space-after="1pt"
                            font-size="10.5pt"
                            font-family="Ludolfus" 
                            ...
                            <fo:leader
                                leader-pattern="dots"/>
                            <fo:basic-link internal-destination="listfigures">
                            <fo:page-number-citation
                                ref-id="listfigures"/>
                            </fo:basic-link>
                            
                        </fo:block>
       case 'introduction' return <fo:block
                            text-align-last="justify"
                            space-after="1pt"
                            font-size="10.5pt"
                            font-family="Ludolfus" 
       ...
       case 'bibliography' return <fo:block
                            text-align-last="justify"
                            space-after="1pt"
                            font-size="10.5pt"
                            font-family="Ludolfus" 
                            ...
                            <fo:leader
                                leader-pattern="dots"/>
                             <fo:basic-link internal-destination="bibliography">
                            <fo:page-number-citation
                                ref-id="introduction"/>
                            </fo:basic-link>
                        </fo:block>
       case 'catalogue' return (<fo:block
                            text-align-last="justify"
                            space-after="1pt"
                            font-size="10.5pt"
                            font-family="Ludolfus" 
              ...
              let $pointer := if ($r/@xml:id) then
                    ...
                    return
                        <fo:block
                            ...
                            margin-left="1cm"
                            font-family="Ludolfus" >
                            <fo:inline font-weight="800">{$signature/text()}</fo:inline>
                            <fo:leader
                                leader-pattern="dots"/>
                            <fo:basic-link
...
<fo:block
             ...
             <fo:block
                        ...
                        <fo:block
                            ...
                            <fo:leader
                                leader-pattern="dots"/>
                            <fo:basic-link internal-destination="IndexWorks"><fo:page-number-citation
                                ref-id="IndexWorks"/>
                                </fo:basic-link>
                        </fo:block>
                        ...
                        <fo:block
                            ...
                            <fo:leader
                                leader-pattern="dots"/>
                            <fo:basic-link internal-destination="IndexSubjects"><fo:page-number-citation
                        ...
                        case 'languages' return
                         <fo:block
                            ...
                            <fo:leader
                                leader-pattern="dots"/>
                            <fo:basic-link internal-destination="IndexLanguages"><fo:page-number-citation
                                ref-id="IndexSubjects"/>
                                </fo:basic-link>
                        </fo:block>
                        case 'keywords' return
                         <fo:block
                            ...
                            <fo:leader
                                leader-pattern="dots"/>
                            <fo:basic-link internal-destination="IndexKeywords"><fo:page-number-citation
       ...
       (<fo:block  
                        ...
                        <fo:block
                            ...
                            <fo:leader
                                leader-pattern="dots"/>
                            <fo:basic-link internal-destination="Appendix"><fo:page-number-citation
                                ref-id="Appendix"/>
                                </fo:basic-link>
                        </fo:block>)
       default return ()
...
<fo:block>
 ...
 (<fo:block font-style="italic" space-after="3mm"  space-before="2mm"  page-break-inside="avoid" page-break-after="avoid">Additional notes</fo:block>,
  <fo:list-block
...
if($lang='ar') then 
<fo:block space-after="3mm" space-before="2mm">{fo:lang('ar')}زخرفة:</fo:block>
else <fo:block font-style="italic" 
space-after="3mm"  space-before="2mm"  
page-break-inside="avoid" page-break-after="avoid">Decoration</fo:block>,
       ( 
      if($lang='ar') then () else ( if($decos//tei:summary) then <fo:block margin-top="3mm" margin-bottom="3mm">{fo:tei2fo($decos/tei:summary)}</fo:block> else()),
    <fo:list-block
        provisional-label-separation="1em"
        provisional-distance-between-starts="2em">
        {if($lang='ar') then fo:lang('ar') else ()}
        {let $decoSele := if($lang='ar') then $decos/tei:decoNote[descendant::tei:*[@xml:lang='ar']] else $decos/tei:decoNote[not(@xml:lang='ar')]
            ...
            return
                <fo:list-item
                    space-after="2mm" >
                    <fo:list-item-label>
                        {if($lang='ar') then() else attribute end-indent{"label-end()"}}
                        <fo:block>{if($lang='ar') then() else $p||')'}</fo:block>
                    </fo:list-item-label>
                    <fo:list-item-body>
                        {if($lang='ar') then() else attribute start-indent{"body-start()"}}
                        {if($lang='ar') then
                        <fo:block>
                        {fo:lang('ar')}
                        {fo:tei2fo($deco/tei:desc[@xml:lang=$lang])}
                        {if($deco/tei:q[@xml:lang=$lang]) 
                          then (<fo:block line-height="12.5pt" baseline-shift="baseline"
                                     font-family="Ludolfus" letter-spacing="0.5pt" font-size="10.5pt">
...
return
(<fo:block font-style="italic" space-after="3mm"  space-before="2mm"  page-break-inside="avoid" page-break-after="avoid">Quire structure</fo:block>,
<fo:block>{$cq} quire{if($cq = 1) then () else 's'}. {
let $quires := for $q at $p in $collation//tei:item 
 order by $q/@xml:id
                ...
                <quire>
                <sequence>{if(matches($q/@n, '[A-Z]')) then string($q/@n) else local:n2roman(xs:integer($q/@n)) }</sequence> <desc><fo:inline font-size="7pt" vertical-align="text-top">{
                let $stubs := if($q/text()[preceding::tei:locus]) 
                            then (let $anastring := analyze-string(string-join($q/text()), '(stub)') 
                            return count($anastring//*:group[@nr=1])) else 0
                let $dimdec := string($q/tei:dim[@unit='leaf']) 
                let $dim := if($stubs ge 1) then (string((xs:integer($dimdec) - $stubs))||'+'||string($stubs)) else $dimdec
                return (
                                $dim|| 
...
declare function fo:paleo($handDesc as element(tei:handDesc)) {
 for $handnote in $handDesc/tei:handNote   return
      <fo:block>Hand {substring-after($handnote/@xml:id, 'h')}{if($handnote/tei:locus) then (' ('||string-join(fo:tei2fo($handnote/tei:locus))||'): ') else (': ')}
...
let $ms := for $m in fo:material($part, $lang)
 return if ($lang='ar') 
            then $m/text()
            else functx:capitalize-first(string($m/@key )) 
return string-join(distinct-values($ms), ' ') 
};

declare function fo:material($part, $lang){
let $thissupport :=  $part//tei:support[1]
return if ($lang='ar') 
            then $thissupport//tei:material[@xml:lang=$lang] 
             else $thissupport//tei:material[not(@xml:lang)]
};
...
let $dimensions :=  if($lang='ar') then $part//tei:dimensions[@type='outer'][@xml:lang=$lang]  else $part//tei:dimensions[@type='outer'][not(@xml:lang)]
return
if($dimensions) then 
    (if($lang='ar') then '،' else ', ') || 
    (if($lang='ar') then 'مم' else ()) || 
    string-join($dimensions/node()/text(), '×') ||' '|| 
    (if($lang='ar') then 'مم' else string($dimensions/@unit)) 
    else ()
};

declare function fo:origDate($part, $lang){
let $od := if($lang='ar') 
                           then $part//tei:origin//tei:origDate[@xml:lang=$lang] 
                           else $part//tei:origin//tei:origDate[not(@xml:lang)]
                        for $origDate in $od
                        let $formatDate := fo:tei2fo($origDate)
                        return ', ' || string-join($formatDate, ' ') 
};
...
if($part[descendant::tei:msPart]) then (
                                    ...
                                    let $date := for $p in $part//tei:msPart 
                                                                    ...
                                                                    return 
                                                                        if($p//tei:origDate[@when or @notBefore or @notAfter]) 
                                                                         then  ( let $od := if($lang='ar') 
                                                                                                         then $p//tei:origin//tei:origDate[@xml:lang=$lang] 
                                                                                                         else $p//tei:origin//tei:origDate[not(@xml:lang)]
                        ...
                        let $decideDate := if(string-length($date) gt 1) 
                                                        ...
                                                        let $generalDate := if($part//tei:origDate[@when or @notBefore or @notAfter]) 
                                                            then fo:origDate($part, $lang) else ()
                                                         let $units :=   for $p in $part//tei:msPart
                                                                                 let $partN := string(substring-after($p/@xml:id, 'p'))
                                                                                return 
                                                                                 if($lang='ar') then () else ('Unit ' || $partN || ': ') || string-join($generalDate, ' ') || '. '
                                                         return 
                                            ' '|| string-join($units)
                                            )
                    return 
...
<fo:block font-style="italic" space-after="3mm"  page-break-inside="avoid" page-break-after="avoid">Main Contents</fo:block>
{for $n in $contents/node() order by count($n/preceding-sibling::node()[name()=$n/name()]) return fo:tei2fo($n)}
</fo:block>,
...
<fo:block space-before="2mm" >
<fo:block font-style="italic" space-after="3mm"  page-break-inside="avoid" page-break-after="avoid">Colophon</fo:block>
...
<fo:block space-before="2mm" >
<fo:block font-style="italic" space-after="3mm"  page-break-inside="avoid" page-break-after="avoid">Binding</fo:block>
{for $bindingNote in $binding//tei:decoNote return fo:tei2fo($bindingNote)}
</fo:block>
else ()
};
...
(<fo:block space-before="2mm" >
<fo:block font-style="italic" space-after="3mm"  page-break-inside="avoid" page-break-after="avoid">Layout</fo:block>
<fo:list-block
        provisional-label-separation="1em"
        provisional-distance-between-starts="2em">
        {for $l in $layoutDesc//tei:layout
            return
                <fo:list-item space-after="3mm">
                    ...
                    <fo:list-item-body
                        start-indent="body-start()">
                        <fo:block>{if($l/tei:locus) then (string-join(fo:tei2fo($l/tei:locus))|| ': ') else ()}{string($l/@columns)} 
                        column{if(number($l/@columns) gt 1) then 's' else()} ({if(matches($l/@writtenLines, '\s')) then replace($l/@writtenLines, '\s', '–') else string($l/@writtenLines)} written lines){fo:tei2fo($l/tei:p)}</fo:block>
                        {if($l/tei:dimensions[not(@type)]) then <fo:block start-indent="10mm" space-before="3mm" space-after="3mm">text area
                        {let $dim := $l/tei:dimensions[not(@type)] return string-join($dim/node()/text(), ' × ')|| string($dim/@unit)}  
                        {let $dimID := string($l/tei:dimensions[not(@type)]/@xml:id) return ' (' || lower-case(fo:tei2fo($l/tei:note[matches(@corresp, $dimID)]/tei:locus)) || ')'}</fo:block> else ()}
                    </fo:list-item-body>
                </fo:list-item>
                }</fo:list-block>
</fo:block>,
...
<fo:block space-before="2mm" >
<fo:block font-style="italic" space-after="3mm">Ruling pattern</fo:block>
<fo:list-block
        provisional-label-separation="1em"
        provisional-distance-between-starts="2em">
...
<fo:block space-before="2mm" >
<fo:block font-style="italic" space-after="3mm"  page-break-inside="avoid" page-break-after="avoid">Palaeography</fo:block>
{fo:paleo($handDesc)}
</fo:block> else ()
};
...
<fo:block space-before="2mm" >
<fo:block font-style="italic" space-after="3mm"  page-break-inside="avoid" page-break-after="avoid">Other details</fo:block>
...
if($h/node()) then <fo:block space-before="2mm" >
<fo:block font-style="italic" space-after="3mm"  page-break-inside="avoid" page-break-after="avoid">History</fo:block>
{for $his at $p in $h return 
<fo:block>{fo:tei2fo($his/node()[not((self::tei:origDate and not(text())))][not(@xml:lang="ar")])}</fo:block>}
</fo:block> else ()
};

declare function fo:URL($mainID){
<fo:block space-before="2mm" space-after="3mm" margin-bottom="16.25pt" page-break-before="avoid">URL: <fo:basic-link
...
{if($partType = '') then attribute id {$part/ancestor::tei:TEI/@xml:id} else ()}
{if($p = 0) then fo:intro($part, '') else ()}
</fo:block>,
if(($p = 0) and ($part[descendant::tei:msPart]) and 
            $part//tei:binding[not(ancestor::tei:msPart)]) 
            then fo:binding($part//tei:binding[not(ancestor::tei:msPart)]) else (),
if(($p = 0) and ($part[descendant::tei:msPart]) and 
            $part//tei:collation[not(ancestor::tei:msPart)]) 
            then fo:collation($part//tei:collation[not(ancestor::tei:msPart)]) else (),
if(($p = 0) and ($part[descendant::tei:msPart])) 
         then ((: we are parsing a msDesc (p=0) and this is a composite manuscript main part, 
        so, do nothing, contents will be done when parsing each part :)) else (
  if($partType != '') 
  then <fo:block space-before="2mm" space-after="3mm">{functx:capitalize-first($partType) || ' '||$p}</fo:block> else (), 
                            fo:contents(($part//tei:msContents)[1]),
    fo:colophon($part),
    if(($part//tei:additions)[1]) then fo:additions(($part//tei:additions)[1]) else (),
    if(($part//tei:binding)[1]) then fo:binding(($part//tei:binding)[1]) else (),
    if(($part//tei:collation)[1] and not($part//tei:material[@key='paper'])) then fo:collation(($part//tei:collation)[1]) else (),
    if(($part//tei:layoutDesc)[1]) then fo:layout(($part//tei:layoutDesc)[1]) else (),
    if(($part//tei:handDesc)[1]) then fo:palaeography(($part//tei:handDesc)[1]) else (),
    if(($part//tei:decoDesc)[1]) then fo:deco(($part//tei:decoDesc)[1], '') else (),
    fo:other($part)
    )
)
};
...
switch($element/name())
     ...
     case 'colophon' return fo:colophon($element)
     case 'additions' return fo:additions($element)
     case 'binding' return fo:binding($element)
     case 'decoDesc' return fo:deco($element, $element/@xml:lang)
     case 'layout' return fo:colophon($element)
     case 'handDesc' return fo:palaeography($element)
...
let $anchor := string($part/@xml:id)
 return 
       ...
       case 'shelfmark' return fo:ifThereIs($part/tei:msIdentifier)
       case 'history' return fo:ifThereIs($part/tei:history)
...
for $part in $msDesc/(tei:msPart|tei:msFrag) return fo:SimpleMsStructureParts($part)
)
};
...
if($part/name() = 'msFrag') 
then substring-after($part/@xml:id, 'f') 
 else if($part/name() = 'msPart') 
         then substring-after($part/@xml:id, 'p')
        else ()
};
...
declare function fo:msheader($msId){
                                        <fo:block id="{string($msId/ancestor::tei:TEI/@xml:id)}" text-align="center" page-break-inside="avoid" page-break-after="avoid" space-before="3mm">
                                            <fo:block
                                            font-weight="800">{$msId/tei:idno[not(@xml:lang)]/text()}</fo:block>
                                               ...
                                               ( '(' , 
                                                ...
                                                order by $prec
                                                return 
                                                <fo:inline>{fo:lang($arabicID/tei:idno/@xml:lang), 
                                                $arabicID/tei:idno/text()|| (if($all = ($prec + 1)) then '' else '; ')
                                                }</fo:inline>
                                                , ')')
                                                else ()} </fo:block>
                                                <fo:block>{
                                               ...
                                               ( '[', 
                                                ...
                                                return 
                                                <fo:inline>{fo:lang($arabicID/tei:idno/@xml:lang), 
                                                $arabicID/tei:idno/text() || (if($all = ($c + 1)) then '' else '; ')
                                                }</fo:inline>
                                                ,
                                                ']')
                                                else ()} </fo:block>
    ...
    order by $num
             return
                    <fo:bookmark
                        ...
                        <fo:bookmark-title> {$shelf}</fo:bookmark-title>
...
declare function fo:sortingkey($input){ string-join($input)
             => replace('ʾ', '')
             =>replace('ʿ','')
             =>replace('\s','')
...
<fo:page-sequence
          ...
          { fo:static('','Index')}
           <fo:flow
                ...
                text-align="justify"
                hyphenate="true">
           <fo:block>This output is based on collaboratively edited data from the project Beta maṣāḥǝft: Manuscripts of Ethiopia and Eritrea (Schriftkultur des christlichen Äthiopiens und Eritreas: eine multimediale Forschungsumgebung). Authoritative names for entries have been contributed by collaborators of the project who may not be listed in the acknowledgements. Please refer to the Identifier to retrieve full informations about these contributors in the website.</fo:block>
            {  for $index in $local:settings/s:indexes/element()     
return 
        if($index = 'yes') then
       switch ($index/name())
...
<fo:block page-break-after="avoid" id="IndexPersons" text-align="justify" >{(attribute font-weight {'700'},
                        ...
                        attribute margin-bottom {'6.25pt'})}Index of Persons</fo:block>,
                                ...
                                group by $r := $ref
                                let $persRecord := if(starts-with($r, '#')) then $local:catalogue//tei:listPerson/tei:person[@xml:id= substring-after($r, '#')] else fo:getFile($r)
                                let $standardname := if(starts-with($r, '#')) then if($persRecord//tei:name) then ($persRecord//tei:name/text()) else ($persRecord//tei:surname/text()||' '|| $persRecord//tei:forename/text()) else fo:printTitleID($r)
                                ...
                                order by $cleanName
                                let $label := string-join($standardname) || (if(count($roles) ge 1) then (', ' || string-join($roles, ', ')) else ())
                               return 
                                <fo:block start-indent="5mm" text-indent="-5mm" margin-bottom="1mm">
                                <fo:block><fo:basic-link 
                                external-destination="https://betamasaheft.eu/{string($r)}"><fo:inline >{string($label)}</fo:inline></fo:basic-link>{if(starts-with($r, '#')) then ': ' else ()}</fo:block>
                                <fo:block start-indent="3mm">{if(starts-with($r, '#')) then () else (string($r)||': ')}
                                {let $nodes := for $att in $personAttestation 
                                                        ...
                                                        return  
                                                            <fo:basic-link internal-destination="{$identif}"><fo:page-number-citation
                                                                                   ref-id="{$identif}"/>{if($att/ancestor::tei:note[ancestor::tei:div[@type='chapter']]) then' n' else ()}</fo:basic-link>
                              
                                 for $n  at $p in $nodes return
                                 ($n, 
                                if(($p = 1) and (count($personAttestation) gt 1)) then ', ' 
                                else if($p = count($personAttestation)) then () 
                                else ', '
                                ) 
                                }</fo:block>
                                </fo:block> }
...
<fo:block page-break-after="avoid"  page-break-before="always"  id="IndexPlaces" text-align="justify" >{(attribute font-weight {'700'},
                        attribute margin-top {'6.25pt'},
                        attribute margin-bottom {'6.25pt'})}Index of Places</fo:block>,
                                ...
                                let $label := $standardname || (if(count($placeType) ge 1) then (', ' || string-join($placeType, ', ')) else ())
                               let $cleanName := fo:sortingkey($standardname)
                                order by $cleanName
                               return 
                                <fo:block start-indent="5mm" text-indent="-5mm" margin-bottom="1mm">
                                <fo:block><fo:basic-link external-destination="{string($r)}"><fo:inline>{string($label)}</fo:inline></fo:basic-link></fo:block>
                                <fo:block>{string($r)}:
                                ...
                                else if($p = count($placeAttestation)) then () 
                                else ', '
                                ) }
                                </fo:block>
                                </fo:block> }
  ...
  case 'works' return
 <fo:block-container text-align="justify">
<fo:block page-break-after="avoid"  page-break-before="always"  id="IndexWorks" text-align="justify">{(attribute font-weight {'700'},
                        attribute margin-top {'6.25pt'},
                        attribute margin-bottom {'6.25pt'})}Index of Texts</fo:block>,
                               ...
                               let $ref := if(contains($WorkAttestation/@ref, '#')) then substring-before($WorkAttestation/@ref, '#') else $WorkAttestation/@ref
                                group by $r := $ref 
                                 ...
                                 let $standardname := if ($workRecord//tei:titleStmt//tei:title[@xml:lang="gez"][@corresp="#t1"][@type="normalized"]) 
                                                                        then $workRecord//tei:titleStmt//tei:title[@xml:lang="gez"][@corresp="#t1"][@type="normalized"][1]/text()
                                                                        else if($workRecord//tei:titleStmt//tei:title[@xml:lang='en'][@corresp="#t1"]) then $workRecord//tei:titleStmt//tei:title[@xml:lang='en'][@corresp="#t1"][1]/text()
                                                                        else fo:printTitleID($r)
                                let $cleanName := fo:sortingkey($standardname)
                                order by $cleanName
                               return 
                                <fo:block start-indent="5mm" text-indent="-5mm" margin-bottom="1mm">
                                <fo:block><fo:basic-link external-destination="{string($r)}"><fo:inline>{if ($workRecord//tei:titleStmt//tei:title[@xml:lang="gez"][@corresp="#t1"][@type="normalized"]) then attribute font-style {'italic'} else ()}{string($standardname)}</fo:inline></fo:basic-link></fo:block>
                                <fo:block start-indent="5mm">CAe {substring($r, 4,4)}: 
                                ...
                                else if($p = count($WorkAttestation)) then () 
                                else ', '
                                )
                                }</fo:block></fo:block> }
                                </fo:block-container>
case 'subjects' return
<fo:block-container text-align="justify">
<fo:block page-break-after="avoid"  page-break-before="always"  id="IndexSubjects" text-align="justify" >{(attribute font-weight {'700'},
                        attribute margin-top {'6.25pt'},
                        attribute margin-bottom {'6.25pt'})}Index of Subjects</fo:block>,
                                  {
                                ...
                                order by $label
                               return if (not(matches($r, '\w+'))) then
                                ()
                                                else
                                                    <fo:block start-indent="5mm" text-indent="-5mm" margin-bottom="1mm">
                                <fo:basic-link external-destination="{string($r)}"><fo:inline>{string($label)}</fo:inline></fo:basic-link>: {$local:prefix}
                              ...
                              for $n at $p in $nodes
                              return  ($n, 
                                if(($p = 1) and (count($sub) gt 1)) then ', ' else if($p = count($sub)) then () else ', ') }</fo:block> }
                                </fo:block-container>

case 'languages' return
<fo:block-container text-align="justify">
<fo:block page-break-after="avoid"  page-break-before="always"  id="IndexLanguages" text-align="justify"  >{(attribute font-weight {'700'},
                        attribute margin-top {'6.25pt'},
                        attribute margin-bottom {'6.25pt'})}Index of Languages</fo:block>,
                                  {
                               ...
                               return 
                                <fo:block start-indent="5mm" text-indent="-5mm" margin-bottom="1mm">
                                <fo:basic-link external-destination="{string($r)}"><fo:inline>{string($r)}</fo:inline></fo:basic-link>: {$local:prefix}  
                                {let $nodes := for $att  in $sub 
                                let $root := $att/ancestor::tei:TEI
                               let $n := string($root/@xml:id)
                               group by $n
...
case 'keywords' return
<fo:block-container text-align="justify">
<fo:block page-break-after="avoid"  page-break-before="always"  id="IndexKeywords" text-align="justify"  >{(attribute font-weight {'700'},
                        attribute margin-top {'6.25pt'},
                        attribute margin-bottom {'6.25pt'})}Index of Keyword</fo:block>,
                                  {
...
return
    <fo:root
       ...
       then fo:layoutmaster('supplements') 
       else fo:layoutmaster('article')}
       {for $settingsValue in $local:settings/s:orderOfParts/element()
       return 
        if($settingsValue = 'yes') then
       switch ($settingsValue/name())
       case 'PDFbookmarks' return  fo:bookmarks() 
       case 'titlePage' return fo:titlepage()
       case 'aknowledgments' return fo:acknow($r/tei:text/tei:front)
       case 'tableofcontents' return fo:table-of-contents()
